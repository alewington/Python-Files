...
# abc	Tools for defining Abstract Base Classes (interfaces for Python classes)
...
# ast	Work with Python code as an Abstract Syntax Tree
# (analyze or transform code).	2.6
# asyncio	Write concurrent code using the async/await syntax
# (event loop, tasks, IO).	3.4
...
# binascii	Convert between binary and ASCII (hex, base64 helpers at C speed).
# bisect	Maintain sorted lists; insert and search with binary search.	2.0
# builtins	Access Python's built-in objects like len, range, and exceptions
# bz2	Read and write bzip2-compressed files and streams.	2.3
# C
# Module	Description	Introduced
# calendar	Work with dates as calendars; print text calendars and compute
# month info.	2.0
...
# codeop	Compile Python code objects conditionally
# (used by code editors and shells).	2.0
# collections	High-performance container datatypes like deque, Counter,
# namedtuple.	2.4
# colorsys	Convert between color systems: RGB, YIQ, HLS, HSV.	2.0
# concurrent	Concurrency framework (futures) for running callables
# asynchronously.	3.2
...
# contextvars	Context-local storage for async code
# (like thread-local, but for tasks).	3.7
...
# D
# Module	Description	Introduced
# dataclasses	Decorator and helpers for classes that store data
# (auto-generate init, repr, etc.).	3.7
# datetime	Dates, times, time zones, and timedeltas made simple and precise.
# dbm	Family of simple on-disk key/value databases (backed by platform libs).
...
# doctest	Test examples embedded in docstrings (keeps docs and code in sync).
# E
# Module	Description	Introduced
...
# F
# Module	Description	Introduced
# faulthandler	Dump Python tracebacks on a crash or on demand
# (helps debug hard faults).	3.3
# filecmp	Compare files and directories to see what changed.	2.0
# fileinput	Loop over lines from stdin or a list of files as a single stream.
...
# functools	Higher-order functions and utilities
# (lru_cache, partial, wraps, etc.).	2.5
# G
# Module	Description	Introduced
...
# locale	Internationalization (i18n) support for formatting numbers and dates.
...
# (not secure against untrusted data).	2.0
...
# U
# Module	Description	Introduced
# unicodedata	Access the Unicode Character Database (properties, normalization)
